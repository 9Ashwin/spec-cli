...
OpenSpec + Superpowers 工作流脚手架工具 — 为人类和 AI Agent 而生。一条命令即可检测 AI 编码平台，并安装 OpenSpec skill、`/opsx:super` 入口 skill 以及 schema 套件。
...
- **一条命令搭好脚手架** — `spec-cli init` 一键安装 OpenSpec skill、入口 skill 和 schema 套件
...
8. 安装 schema 套件到 `openspec/schemas/<name>/`
