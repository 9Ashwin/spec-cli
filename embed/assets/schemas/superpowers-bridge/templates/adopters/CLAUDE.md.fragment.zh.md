...
<!-- 如果你定制 schema 名称或 bridge repo URL，请对应修改；否则保持原样即可。 -->

## 变更工作流（Claude Code 启动先读）

本 repo 采用 [`superpowers-bridge`](https://github.com/JiangWay/openspec-schemas/tree/main/superpowers-bridge) 桥接 OpenSpec 与 Superpowers。整合规则（语言、artifact 路径、PRECHECK）以该 bridge README 为准；以下是给 Claude 的 routing 指引。

### 入口分流

| 你看到的触发 | 应该怎么做 |
|---|---|
| 用户用 narrative 发起「设计讨论 / 头脑风暴」 | 先口头 `superpowers:brainstorming`，**不**写到 `docs/superpowers/specs/`；对话收敛后依下方 5 条判准升级到 `/opsx:propose` |
...
详见 [superpowers-bridge README §进入与离开的判断](https://github.com/JiangWay/openspec-schemas/blob/main/superpowers-bridge/README.zh-TW.md#进入与离开的判断entry--exit-gates)。
