...
如果用户以 narrative(“我们来讨论架构”“头脑风暴一下”)触发了 `brainstorming`,brainstorming 的生成**不可以**写到 `docs/superpowers/specs/` —— 那会绕过本 schema 的 output redirection,在 repo 里留下 orphan artifact。
...
flowchart TD
    ...
    subgraph Plan ["📝 PLANNING — 7 个 artifact"]
        direction TB
        BS["<b>brainstorm.md</b><br/><i>brainstorming</i>"]
        PROP["<b>proposal.md</b>"]
        DES["<b>design.md</b><br/><i>(必填,结构化决策)</i>"]
        SP["<b>specs/**/*.md</b>"]
        TK["<b>tasks.md</b>"]
        PL["<b>plan.md</b><br/><i>writing-plans</i>"]
    ...
    subgraph Apply ["⚙️ APPLY — 7 个顺序步骤(requires: plan,tracks: tasks.md)"]
        direction TB
        ...
        A5["<b>5. Archive</b><br/><i>openspec archive <change-name> -y</i><br/>(sync delta + 移动目录)"]
...
APPLY ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  2. subagent-driven-development(+ TDD + code-review 传递)
  3. openspec-verify-change → verify.md ◄┐
                              │           │ blocking → 回去修
                              ▼           │
  4. retrospective.md(PR 之前;hot context)
  5. openspec archive <change-name> -y(sync delta + 移动目录)
  6. finishing-a-development-branch(🏁 PR 是最后一步)
```
...
|---|---|---|---|
| 1 | `brainstorming` | `brainstorm` artifact instruction | 直接(含 PRECHECK) |
| 2 | `writing-plans` | `plan` artifact instruction | 直接(含 PRECHECK) |
| 3 | `using-git-worktrees` | apply step 1 | 直接 |
| 4 | `subagent-driven-development` | apply step 2 | 直接 |
| 5 | `test-driven-development` | (#4 内部触发) | **传递** |
| 6 | `requesting-code-review` | (#4 内部触发) | **传递** |
| 7 | `finishing-a-development-branch` | apply step 4 | 直接 |

加上一个 OpenSpec built-in:`openspec-verify-change`(apply step 3,生成 `verify.md`)。

> **不支持 `executing-plans` fallback**。本 schema 是 opinionated 的:要求 subagent-capable 平台(Claude Code、Codex 等)。替代 executor `executing-plans` 并**不会** transitively 触发 TDD 或 code-review(已对 [SKILL.md](https://github.com/obra/superpowers/blob/main/skills/executing-plans/SKILL.md) 做事实查核 —— body 完全没提到 TDD 或 code-review,Integration 段也未列出 `test-driven-development` 与 `requesting-code-review`)。退到 2b 等于静默降级 Superpowers 的核心价值。若你的平台没有 subagent 支持,改用 OpenSpec 内建的 `spec-driven` schema。
...
- `using-git-worktrees`
- `subagent-driven-development`(传递依赖:`test-driven-development`、`requesting-code-review`)
- `finishing-a-development-branch`

skill 缺失 → STOP 并通知用户,不静默 fallback,本 schema 内也没有 manual mode。建议用户在那个 change 改用 OpenSpec 内建的 `spec-driven` schema,或安装缺失的 skill 后重来。
...
#### 1. Workspace — `using-git-worktrees`
...
- **TDD**(`test-driven-development`):先写失败测试 → 看着它 fail → 写最小代码 → pass;production code 写在没测试之前会被删掉重来
- **per-task code review**(`requesting-code-review`):spec compliance review + code quality review;Critical 级问题挡下进度

完成 coarse task 就更新 `tasks.md` checkbox。所有 task 执行完后,对整个 implementation 再做一次 final code review。

本 schema **不支持** `executing-plans` 作为 fallback。理由见下方“六个值得记住的设计触点”段。
...
#### 5. Archive — `openspec archive <change-name> -y`(或 `/opsx:archive <change-name>`)

必须在包含最新已勾选 `tasks.md`、`verify.md`、`retrospective.md` 和 implementation commits 的同一个 branch/worktree 中运行。不要从 stale main checkout 执行 archive。它会把 delta spec sync 到 `openspec/specs/<capability>/spec.md`、把 change 目录移动到 `openspec/changes/archive/YYYY-MM-DD-<name>/`。在开 PR **之前**执行完,这样 PR diff 反映完整的 archived cycle 状态(所有 artifact 完成、spec 已 sync、folder 在 `archive/`)。

#### 6. Completion — `finishing-a-development-branch`
...
每个 invoke Superpowers skill 的 artifact / apply step 在 instruction 开头跑 PRECHECK,确认 skill 真的存在于 LLM 的 available skills list。**缺失就 STOP,不静默 fallback**。这是 [PR #970 review](https://github.com/Fission-AI/OpenSpec/pull/970) 顾虑 #1 第 1 层的具体应对 —— 明确失败, fail early。
...
### 4. Opinionated:只支持 subagent 平台,没有手动 fallback

本 schema 要求 subagent-capable 平台(Claude Code、Codex 等)。替代 executor `executing-plans` **不会** transitively 触发 TDD 或 code-review(已对其 [SKILL.md](https://github.com/obra/superpowers/blob/main/skills/executing-plans/SKILL.md) 做事实查核 —— body 完全没提及这两者,Integration 段也未列出 `test-driven-development` 与 `requesting-code-review`)。退到 2b 等于静默丢掉 Superpowers 带给整合的核心价值。我们选择在 Step 0 明确失败,并指引用户改用内建的 `spec-driven` schema。
...
契约分三层 — **基准声明 + 自动 drift 检测 + 人类检核** — 不是自动兼容性 enforcement。
...
- `tasks.md` → 跟踪整体进度(apply phase 的 `tracks` 字段解析 checkbox)
- `plan.md` → 指导 subagent 逐步实现(executor 的输入)

apply 要求 `plan` 而非 `tasks`,因为 executor 需要 micro-step 才能有效工作;`tracks: tasks.md` 确保进度仍由粗粒度 checkbox 跟踪。
